...
# استقبال الـ Webhook على أي مسار يرسله التلقرام لحل مشكلة الـ 404 نهائياً
@app.route('/<path:text>', methods=['POST'])
@app.route('/', methods=['POST'])
def webhook(text=None):
    if request.method == "POST":
        ...
        if "message" in data and "text" in data["message"]:
            ...
            # تفقد نص الرسالة والرد بناءً عليها
            if user_text == "/start":
                reply = "أهلاً بك! أنا جاهز لتنفيذ طلباتك. ماذا تريد مني أن أفعل؟"
            else:
                # يمكنك لاحقاً تبديل هذا السطر بربط البوت بالذكاء الاصطناعي الخاص بك
