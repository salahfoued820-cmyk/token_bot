...
if st.button("🤖 اصنع الكود البرمجي الآن"):
    if user_idea.strip():
        with st.spinner("جاري تحليل الفكرة وهندسة الكود البرمجي..."):
            try:
                # استخدام البوابة البرمجية المباشرة لـ MyMemory لترجمة وتوليد الطلب
                ...
                if response.status_code == 200:
                    ...
                    if res_json.get("responseData"):
                        ...
                        # التثبيت والصياغة داخل قالب الماركداون البرمجي لعرض الكود الصافي الجاهز للنسخ بنقاء 100%
                        if generated_code and "doctype" not in generated_code.lower():
                            st.success("✨ **تم توليد الكود البرمجي بنجاح وبأعلى جودة :**")
                            st.code(generated_code, language="python")
                        else:
                            st.warning("⚠️ استجاب محرك البرمجة ولكن النتيجة جاءت مبهمة، أعد صياغة طلبك واضغط مجدداً.")
                    else:
